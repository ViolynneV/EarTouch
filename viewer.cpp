...
pthread_t Viewer::thread, Viewer::update_threads[2];
...
const int MIN_X = 130, MAX_X = 950, MIN_Y = 0, MAX_Y = 1215;
const int SWIPE_LEFT_X = 300, SWIPE_UP_Y = 500;
...
void resize(Point& p, int lx, int rx, int ly, int ry)
{
	if (p.x < lx) p.x = lx;
	if (p.x > rx) p.x = rx;
	if (p.y < ly) p.y = ly;
...
bool findPattern(Mat& m, Rect& pattern, RotatedRect& firstTouch)
{
	...
	*/
	...
	if (contours[i].size() > 100 && contours[i].size()) {
			rect = true;
...
void Viewer::displayFrameCV(Frame &frame) {
	...
	int sum = matSum<float>(input);

	//if (sum < 100) { return; }


	Point newsum(frame.frameID, sum);
	//cout << "* * * " << frame.frameID << " " << sum << " " << endl << endl;
	...
	bool isDirty = (sum > 15000) && isStable;
	//bool isDirty = (sum > 10000);
	...
	/*for (int i = 0; i < deltas_buffer.size(); i++) {
		cout << deltas_buffer.at(i)<<" ";
	}
	cout << endl << endl;*/

	//cout << "leavesum" << leavesum << endl;

	if (last_dirty) {
		//isDirty = (sum > 10000) && (leavesum > -7000);
	...
	if (isDirty)
	{
		...
		if (!last_dirty)                                         //触摸开始
		{
			frame_count = 0;
			Rect rectsum;
			if (findPattern(binaryImage, rectsum, firstTouch))
			{
				
				Point2f vertices[4];
				firstTouch.points(vertices);
				Mat tmp = Mat::zeros(Size(360, 640), CV_8UC3);
				cvtColor(binaryImage, tmp, CV_GRAY2RGB);

				for (int i = 0; i < 4; i++) {
					line(tmp, vertices[i], vertices[(i + 1) % 4], Scalar(255, 0, 0),2,1);

				}

				Point want;
				calcFirstPoint(firstTouch, want);
				resize(want, 0, 320, 0, 560);
				circle(tmp, want, 5, Scalar(0, 0, 255), -1);




				pattern = binaryImage(rectsum);

				trackerInit(tracker[0], binaryImage, rectsum);
				trackerInit(tracker[1], binaryImage, rectsum);

				Point cp;
				gravityCenter(pattern, cp);
				// the first point needn't be rotated
				//Point real_result((rectsum.x + cp.x) / 320.0 * 1080.0, (rectsum.y + cp.y) / 560.0 * 1920.0);
				Point real_result(want.x / 320.0 * 1080.0, want.y / 560.0 * 1920.0);

				//record the first point

				if (firstTouch.size.height < 250 && firstTouch.size.width < 250) {
					firstPoint.x = rectsum.x + cp.x;
					firstPoint.y = rectsum.y + cp.y;
				}
				else {
					firstPoint.x = want.x;
					firstPoint.y = want.y;
				...
				circle(tmp, Point(rectsum.x + cp.x, rectsum.y + cp.y), 5, Scalar(255, 0, 255), -1);
				...
				if (relative) {
					...
					if (firstTouch.size.height < 250 && firstTouch.size.width < 250) {
						result = last_result = Point(rectsum.x + cp.x, rectsum.y + cp.y);
					}
					else {
				...
				// no need to rotate
				rotatedResult.x = result.x;
				rotatedResult.y = result.y;

				if (result.x < MIN_X || result.x > MAX_X || result.y < MIN_Y || result.y > MAX_Y || result.x < SWIPE_LEFT_X || result.y < SWIPE_UP_Y)
				{
					if ((start_point.x >= MIN_X && start_point.x >= SWIPE_LEFT_X && start_point.y < MAX_Y) && ((result.x >= MIN_X && result.x <= MAX_X && result.x >= SWIPE_LEFT_X && result.y >= SWIPE_UP_Y) || result.y > MAX_Y))
						m_inject.vibrate();
					return;
				}
				last_point[0].x = last_point[1].x = rectsum.x;
				last_point[0].y = last_point[1].y = rectsum.y;
		...
		else                                            //触摸中
		{
			...
			if (succ)
			{
				if (!succ_last)
				{
					Rect rect;
                    RotatedRect tmp;
					findPattern(binaryImage, rect, tmp);
				...
				resize(ptmp, 0, 320, 0, 560);
				result = ptmp;

				//has to be rotated

				rotateAxis(firstPoint, result, 0.0, rotatedResult);
				...
				//result with rotated
				rotatedResult.x = rotatedResult.x / 320.0 * 1080.0;
				rotatedResult.y = rotatedResult.y / 560.0 * 1920.0;
			...
			else
			{
				...
				Rect rect;
                RotatedRect tmp;
				if (!findPattern(binaryImage, rect, tmp))
				{
					if (now)
						now = 0, last = 1;
					else now = 1, last = 0;
				...
				else
				{
					...
					if (relative)
						result = ptmp = box_point - last_box_point + last_result;
					else result = ptmp = box_point - last_box_point + last_result;
			...
			{
				if ((start_point.x >= MIN_X && start_point.x >= SWIPE_LEFT_X && start_point.y < MAX_Y) && ((result.x >= MIN_X && result.x <= MAX_X) || result.y > MAX_Y))
					m_inject.vibrate();
				return;
			}
		...
		ropoints_buffer.push_back(rotatedResult);
		...
		//for ropoints_buffer
		//int M = ropoints_buffer.size();
		//if (M >= 5)
		//{
		//	cubicSmooth5(ropoints_buffer);
		//	if (M == 5)
		//	{
		//		Point p = ropoints_buffer[0];
		//		p.x = (float)(p.x - MIN_X) / (MAX_X - MIN_X) * 1080;
		//		p.y = (float)(p.y - MIN_Y) / (MAX_Y - MIN_Y) * 1920;
		//		//m_inject.touch_down(p.x, p.y);
		//		sent(p.x, p.y, "down");
	...
	else                                               //触摸结束
	{
		...
		//if (has_start && end_point.y >= MAX_Y && start_point.y >= MAX_Y)
		if (has_start && ((end_point.y <= SWIPE_UP_Y && start_point.y <= SWIPE_UP_Y) || (end_point.x <= SWIPE_LEFT_X && start_point.x <= SWIPE_LEFT_X)))
		{
			cout << start_point << ' ' << end_point << endl;
			int dx = end_point.x - start_point.x, dy = end_point.y - start_point.y;
			if (dx * dx + dy * dy > 10000)
			{
				if (!(end_point.y <= SWIPE_UP_Y && start_point.y <= SWIPE_UP_Y && end_point.x <= SWIPE_LEFT_X && start_point.x <= SWIPE_LEFT_X))
				{
					if (end_point.y <= SWIPE_UP_Y && start_point.y <= SWIPE_UP_Y && (end_point.x >= SWIPE_LEFT_X || start_point.x >= SWIPE_LEFT_X) && abs(dx) > abs(dy))
					{
						int fx = (start_point.x < end_point.x ? max(0, start_point.x - 100) : min(1079, start_point.x + 100)), tx = (start_point.x < end_point.x ? min(1079, end_point.x + 100) : max(0, end_point.x - 100));
						//m_inject.swipe(fx, 1000, tx, 1000);
						cout << "double click\n";
						m_inject.touch_down(500, 500);
						m_inject.touch_up();
						cout << (float)clock() / CLOCKS_PER_SEC << endl;
						m_inject.touch_down(500, 500);
						m_inject.touch_up();
					...
					if (end_point.x <= SWIPE_LEFT_X && start_point.x <= SWIPE_LEFT_X && (end_point.y >= SWIPE_UP_Y || start_point.y >= SWIPE_UP_Y) && abs(dx) < abs(dy))
					{
						int fy = (start_point.y < end_point.y ? max(0, start_point.y - 200) : min(1919, start_point.y + 100)), ty = (start_point.y < end_point.y ? min(1919, end_point.y + 100) : max(0, end_point.y - 200));
						m_inject.swipe(500, fy, 500, ty);
					}
				}
			}
			...
			//else
			//{
			//	last_result = Point(160, 180);
			//	//relative = !relative;
			//}
		}
		start_point = end_point = Point(0, 0);
		has_start = false;
		points_buffer.clear();
