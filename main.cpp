...
int main() {
	...
	while (true) {
		...
		if (waitKey(1) == 27 || index == file_buffer.size()) {
			break;
		}
		...
		for (int i = 0; i < Viewer::stablepoints_buffer.size(); i++) {
			...
			char tam[100];
			sprintf_s(tam,"%f", Viewer::stablepoints_buffer[i].y);
			cout << "--------------------------------" << Viewer::stablepoints_buffer[i].y << endl << endl;
			putText(sum,tam, Viewer::stablepoints_buffer[i],FONT_HERSHEY_SIMPLEX,0.4,cvScalar(255,0,255),1);
		}
		imshow("sum", sum);
